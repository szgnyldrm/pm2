...
God.executeApp = function executeApp(env, cb) {
  ...
  env_copy["status"] = env.autostart
    ? cst.LAUNCHING_STATUS
    : cst.STOPPED_STATUS;
...
God.handleExit = function handleExit(clu, exit_code, kill_signal) {
  ...
  var stopExitCodes =
    proc.pm2_env.stop_exit_codes !== undefined &&
    proc.pm2_env.stop_exit_codes !== null
  ...
  var stopping =
    ...
    stopExitCodes
      .map((strOrNum) =>
        typeof strOrNum === "string" ? parseInt(strOrNum, 10) : strOrNum
      )
      .includes(exit_code);
